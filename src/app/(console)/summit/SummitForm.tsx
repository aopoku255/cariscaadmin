...
import styles from '../cpd/cpd.module.css';
...
export function SummitForm({
  ...
  event?: AdminSummitEvent; countries: ReferenceData['countries']; apiBase: string;
  certificateTemplates?: CertificateTemplate[];
}) {
  const [state, formAction] = useActionState(
    event ? updateSummitAction : createSummitAction,
    emptyAdminState,
  );
  ...
  const cfpOpens = parts(event?.summit?.callForPapersOpensAt);
  const cfpCloses = parts(event?.summit?.callForPapersClosesAt);
  ...
  return (
    <form action={formAction} className={styles.form}>
      ...
      <fieldset className={styles.group}>
        <legend className={styles.groupTitle}>Call for papers</legend>
        <p className={styles.groupNote}>
          Leave the dates blank to accept submissions for as long as the call
          stays open — there is no automatic cutoff without one.
        </p>

        <Field label="Theme" htmlFor="theme" hint="e.g. Resilient Supply Chains for Africa.">
          <input id="theme" name="theme" className={inputClass} defaultValue={event?.summit?.theme ?? ''} />
        </Field>

        <div className={styles.pair}>
          <Field label="Call opens" htmlFor="cfpOpensDate">
            <div className={styles.inline}>
              <input id="cfpOpensDate" name="cfpOpensDate" type="date" className={inputClass}
                defaultValue={cfpOpens.date} />
              <input name="cfpOpensTime" type="time" className={inputClass}
                defaultValue={cfpOpens.time || '00:00'} aria-label="Call for papers opening time" />
            </div>
          </Field>
          <Field label="Call closes" htmlFor="cfpClosesDate">
            <div className={styles.inline}>
              <input id="cfpClosesDate" name="cfpClosesDate" type="date" className={inputClass}
                defaultValue={cfpCloses.date} />
              <input name="cfpClosesTime" type="time" className={inputClass}
                defaultValue={cfpCloses.time || '23:59'} aria-label="Call for papers closing time" />
            </div>
          </Field>
        </div>

        <Field label="Keynote speakers" htmlFor="keynoteCount" hint="A headline count shown on the event page. Leave blank if not decided yet.">
          <input id="keynoteCount" name="keynoteCount" type="number" min={0} className={inputClass}
            defaultValue={event?.summit?.keynoteCount ?? ''} />
        </Field>
      </fieldset>
      ...
      <div className={styles.formActions}>
        ...
        <Link href={event ? `/summit/${event.id}` : '/summit'} className={styles.cancel}>
          Cancel
        </Link>
      </div>
    </form>
